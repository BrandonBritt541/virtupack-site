...
(function () {
  ...
  try {
    ...
    // ── FLAPS ─────────────────────────────────────────────────
    // Flaps pivot from their inner edge at the top of the box (y=90).
    // We create each flap as a BoxGeometry centered at origin,
    // then parent it to a pivot object positioned at the hinge.
    ...
    function addFlap(isLong, pivotX, pivotZ, hingeAxis, angle) {
      ...
      // Center the flap so its inner edge is at pivot: shift by half depth
      m.position.set(0, 0, flapD / 2);
      pivot.add(m);
      solidMeshes.push(m);

      // Rotate pivot on hinge axis
    ...
    // Front flap — hinges at z=+100, opens forward/outward (+x rotation)
    const fp1 = addFlap(true,  0,  100, 'x',  Math.PI * 50 / 180);
    // Back flap — hinges at z=-100, opens outward (flipped)
    ...
    // Left flap — hinges at x=-100, opens left
    ...
    // Right flap — hinges at x=+100, opens right
    ...
    paperClumps.forEach(({ x, z, sx, sy, sz }) => {
      ...
      // y=-50 sits the paper inside the box, some peeking above y=90
    ...
    product.position.set(0, 15, 0);  // sits in paper, peeks ~30 above box top
    group.add(product);  // NOT in solidGroup — survives hide
    ...
    function addEdgeWire(geo, color, targetOpacity, parent, px, py, pz, rx, ry, rz) {
      ...
      // Glow copy (slightly larger, lower opacity)
    ...
    // Flap wires — replicate same pivot transforms
